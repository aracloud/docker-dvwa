...
sed -i "s/= 'impossible'/ = 'low'/g" /app/config/config.inc.php && \
sed -i 's/FileInfo/All/g' /etc/apache2/sites-available/000-default.conf && \
sed -i 's/root/admin/g' /app/config/config.inc.php && \
sed -i "/SERVER_NAME/a \$SERVER_ADDR      = 'Web Server SERVER_IPADDR: <em>' . \$_SERVER[ 'SERVER_ADDR' ] . '</em>';" /app/dvwa/includes/dvwaPage.inc.php && \
sed -i '/SERVER_NAME/a \ \ \ \ \ \ \ \ {\$SERVER_ADDR}<br />' app/setup.php && \
